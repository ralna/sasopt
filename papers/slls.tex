...
over the intersection of $\ns \geq 1$ non-overlapping unit simplices,
\eqn{si}{\Delta_j^{n_j} \define
 \{ x \st \sum_{i \in \calI_j} x_i = 1, \; x_i \geq 0, \; i \in \calI_j\}, \;\;
 j = 1, \ldots, \ns,}
where
\eqn{is}{\bigcup_{j=1}^{\ns} \calI_j \subseteq \calN \tim{and}
\calI_{j_1} \cap \calI_{j_2} = \emptyset \tim{for} 1 \leq j_1 < j_2 \leq \ns.}
This allows the possibility that some variable $x_i$ are not constrained to
lie within any of the simplices, and we denote the indices of these
by $\calI_0 \define \calN \setminus \cup_{j=1}^{\ns} \calI_j$.
The problem is of interest as this
is precisely the case for small-angle scattering applications with
$\ns$ distinct sets of polydisperse parameters, plus a pair of unconstrained 
...
the projection may be found block-componentwise according to the index sets
$\calI_j$ (and the projection for any unconstrained variables trivially leaves
them alone).
